# 레벨플레이 미디에이션 튜토리얼
<p align="center">
  <img src="https://github.com/goldmetal/unity.3d.levelplay-tutorial/blob/main/Banner.png"><br>
  <img src="https://img.shields.io/badge/Unity-6000.5.3f1-000000?&logo=unity&logoColor=white" style="margin: 0 5px;">
  <img src="https://img.shields.io/badge/Levelplay-2B7361?&logo=unity&logoColor=white" style="margin: 0 5px;">
  <img src="https://img.shields.io/badge/Voxel_Art-A0BB16?&logoColor=white">
</p>
유니티의 광고 최적화 플랫폼, 레벨플레이를 활용하여 <br>
쉽고 빠르게 모바일 게임 어플리케이션에 광고를 추가하고 수익화를 배우는 예제 프로젝트입니다.  
  
### 미디에이션이란?  
최적의 광고를 나만의 게임 어플리케이션이 가진 광고 유닛에 노출하여  
수익을 최적화 하는 방법을 의미합니다.  
  
### 장면 소개
장면은 1개로 구성하고 있으며 복셀 그래픽 기반의 간단한 캐주얼 게임 로직을 갖추고 있습니다.  
레벨플레이 SDK 제작 실습은 **AdManager.cs**에서 이루어집니다.  
  
### 학습 방법
**Unite Seoul 2026 Hands-On** 트레이닝에서 **@Chris-Won**님과 함께 오프라인으로 학습이 진행되며  
오른쪽의 **프로젝트 다운로드**로 들어가서 **unity_2d_levelplay_tutorial_6000_5_3f1.zip**을 다운로드합니다.  
이후 압축을 풀고 미리 설치된 유니티 허브에서 프로젝트를 추가하여 시작할 수 있습니다.
  
### 등장 인물
해당 프로젝트에 등장하는 3종 캐릭터는 다음 에셋에서 만날 수 있습니다.
- **Sophie**: **[Practical Game Accessibility](https://assetstore.unity.com/packages/essentials/tutorial-projects/practical-game-accessibility-case-study-223056)**
- **Kyle**: **[Robot Kyle](https://assetstore.unity.com/packages/3d/characters/robots/robot-kyle-urp-4696)**
- **Unitychan**: **[Unity-Chan!](https://assetstore.unity.com/packages/3d/characters/unity-chan-model-18705)**
  
  
#### Special Thanks
**[@Chris-Won](https://github.com/chris-is-one)**: 미디에이션과 레벨플레이 지식을 알려주신 유니티 서포트 엔지니어입니다.
